...
void WCSimOpBoundaryProcess::CoatedDielectricDielectric_Model2()
{
  ...
  G4MaterialPropertiesTable* MPT = Material2->GetMaterialPropertiesTable();
  G4double Rindex2 = 0.0;  // Define Rindex2 here
  ...
  G4double fCoatedRindex = 0.0;
  G4double fCoatedRindexIm = 0.0;
  G4double fCoatedThickness = 0.0;
  ...
  G4double wavelength = h_Planck * c_light / thePhotonMomentum;
  G4double PdotN;
  G4double cost1, sint1, cost2, sint2; // Define these variables
  G4double E1_perp, E1_parl;
...
n2 = Rindex1;
n3 = fCoatedRindex - G4complex(0, fCoatedRindexIm);
n4 = Rindex2;
...
do {
  ...
  if (std::abs(cost1) < 1.0 - kCarTolerance)
    {
      sint1 = std::sqrt(1.0 - cost1 * cost1);
      sint2 = sint1 * Rindex1 / Rindex2;
      sintTL = sint1 * Rindex1 / fCoatedRindex;
    }
  else
  ...
  costh3 = std::sqrt(1.0 - sinth3 * sinth3);
  if (cost1 < 0)
    costh3 *= -1;
  costh4 = std::sqrt(1.0 - sinth4 * sinth4);
  if (cost1 < 0)
    costh4 *= -1;

  // reflection and transmission coefficients (TM mode)
  r23p = (n2 * costh3 - n3 * costh2) / (n2 * costh3 + n3 * costh2);
  t23p = 2.0 * n2 * costh2 / (n2 * costh3 + n3 * costh2);
  t32p = 2.0 * n3 * costh3 / (n2 * costh3 + n3 * costh2);
  r34p = (n3 * costh4 - n4 * costh3) / (n3 * costh4 + n4 * costh3);
  t34p = 2.0 * n3 * costh3 / (n4 * costh3 + n3 * costh4);

  // reflection and transmission coefficients (TE mode)
  r23s = (n2 * costh2 - n3 * costh3) / (n2 * costh2 + n3 * costh3);
  t23s = 2.0 * n2 * costh2 / (n2 * costh2 + n3 * costh3);
  t32s = 2.0 * n3 * costh3 / (n2 * costh2 + n3 * costh3);
  r34s = (n3 * costh3 - n4 * costh4) / (n3 * costh3 + n4 * costh4);
  t34s = 2.0 * n3 * costh3 / (n3 * costh3 + n4 * costh4);

  delta = -2.0 * pi * fCoatedThickness * n3 / wavelength * costh3;

  rp = r23p + (t23p * t32p * r34p * exp(2.0 * i * delta)) / (1.0 + r23p * r34p * exp(2.0 * i * delta));
  tp = t23p * t34p * exp(i * delta) / (1.0 + r23p * r34p * exp(2.0 * i * delta));
  Rp = real(rp * conj(rp));
  Tp = real(n4 * costh4 / n2 / costh2 * tp * conj(tp));

  rs = r23s + (t23s * t32s * r34s * exp(2.0 * i * delta)) / (1.0 + r23s * r34s * exp(2.0 * i * delta));
  ts = t23s * t34s * exp(i * delta) / (1.0 + r23s * r34s * exp(2.0 * i * delta));
  Rs = real(rs * conj(rs));
  ...
  if (rdm < Rtot)
      ...
      else
	{
      ...
      PdotN = OldMomentum * theFacetNormal;
      NewMomentum = OldMomentum - (2.0 * PdotN) * theFacetNormal;
      ...
      else
	{
      ...
      if (sint1 > 0.0)
	{
	  cost2 = std::sqrt(1.0 - sint2 * sint2);
	  if (cost1 < 0.0)
	    cost2 *= -1;
	  alpha = cost1 - cost2 * (Rindex2 / Rindex1);
	  NewMomentum = OldMomentum + alpha * theFacetNormal;
  ...
  OldMomentum = NewMomentum.unit();
  OldPolarization = NewPolarization.unit();
 } while (NewMomentum * theGlobalNormal > 0.0); // Fix the loop termination condition
}
