	      ...
	      for (G4int i = 0; i < nProcesses; i++)
		{
		  ...
		  //		  if (opBoundary)
		      ...
		      if(boundaryStatus == 10)
			{
			  ...
			  //			  trackStatus[trackID] = "Absorbed";
			}
		      
		      else if((boundaryStatus == 2 || boundaryStatus == 42 || boundaryStatus == 43)) // && track->GetCurrentStepNumber() == 2)
                        {
			      fEventAction->IncrementNumTransmitted();
			      //			      trackStatus[trackID] = "Transmitted";
                        }
		      //		      else if((boundaryStatus == 40)) //&& track->GetCurrentStepNumber() == 2)
		      else if((boundaryStatus == 9 || boundaryStatus == 4 || boundaryStatus == 5 || boundaryStatus == 6 || boundaryStatus == 7 || boundaryStatus == 8 || boundaryStatus == 41))
                        {
			      fEventAction->IncrementNumReflected();
			      //			      trackStatus[trackID] = "Reflected";
                        }
