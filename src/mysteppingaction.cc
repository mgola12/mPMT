...
MySteppingAction::MySteppingAction() : stepCount(0)
{
...
void MySteppingAction::UserSteppingAction(const G4Step *step)
{
  ...
  volume.push_back(volumeName);
  
  G4VPhysicalVolume* preStepVolumePhy = step->GetPreStepPoint()->GetPhysicalVolume();
  G4VPhysicalVolume* postStepVolumePhy = step->GetPostStepPoint()->GetPhysicalVolume();

  if(preStepVolumePhy && postStepVolumePhy)
    {
	 G4String preStepVolumeName = preStepVolumePhy->GetName();
	 G4String postStepVolumeName = postStepVolumePhy->GetName() ;

	 preStepVolume.push_back(preStepVolumeName);
	 postStepVolume.push_back(postStepVolumeName);
    }
  /*
  ...
  */
  ...
  stepNumbers.push_back(currentStepNumber);
  /////////////////////////////////////////////////////////////////
  ...
  if(track->GetCurrentStepNumber() == 1) 
    {
      ...
      G4cout<<"THE INCIDENT ANGLE IS :  "<<incidenceAngle<<G4endl;
    }
  ///////////////////////////////////////////////////////////////////////////////////
  ...
  if (preVol && postVol &&
    ...
    // Check for the photocathode surface specifically
    bool isAtPhotocathode =
      (preVolName == "pmtSphere" && postVolName == "pmtInnerSphere") ||
      (preVolName == "pmtInnerSphere" && postVolName == "pmtSphere");
    
    if (!isAtPhotocathode) return;

    static WCSimOpBoundaryProcess* opBoundary = nullptr;

    // Only retrieve the boundary process once (first event)
    if (!opBoundary) {
      ...
      for (G4int i = 0; i < nProcesses; ++i) {
        if ((*pv)[i]->GetProcessName() == "OpBoundary") {
          opBoundary = dynamic_cast<WCSimOpBoundaryProcess*>((*pv)[i]);
          break;
        }
      }
    }
    ...
    if (opBoundary) {
      WCSimOpBoundaryProcessStatus boundaryStatus = opBoundary->GetStatus();
      
      G4cout << "[PHOTOCATHODE] Status = " << boundaryStatus
	     << " | From: " << preVolName << " → " << postVolName << G4endl;
      
      if (boundaryStatus == Absorption) {
	fEventAction->IncrementNumAbsorbed();
      } else if (boundaryStatus == Detection) {
	fEventAction->IncrementNumDetected();
      } else if (boundaryStatus == Transmission ||
		 boundaryStatus == StepTooSmall ||
		 boundaryStatus == SameMaterial ||
		 boundaryStatus == CoatedDielectricFrustratedTransmission) {
	fEventAction->IncrementNumTransmitted();
      } else if (
		 ...
		 boundaryStatus == BackScattering ||
		 boundaryStatus == PolishedLumirrorAirReflection ||
		 boundaryStatus == PolishedLumirrorGlueReflection ||
		 boundaryStatus == PolishedAirReflection ||
		 boundaryStatus == PolishedTeflonAirReflection ||
		 boundaryStatus == PolishedTiOAirReflection ||
		 boundaryStatus == PolishedTyvekAirReflection ||
		 boundaryStatus == PolishedVM2000AirReflection ||
		 boundaryStatus == PolishedVM2000GlueReflection ||
		 boundaryStatus == EtchedLumirrorAirReflection ||
		 boundaryStatus == EtchedLumirrorGlueReflection ||
		 boundaryStatus == EtchedAirReflection ||
		 boundaryStatus == EtchedTeflonAirReflection ||
		 boundaryStatus == EtchedTiOAirReflection ||
		 boundaryStatus == EtchedTyvekAirReflection ||
		 boundaryStatus == EtchedVM2000AirReflection ||
		 boundaryStatus == EtchedVM2000GlueReflection ||
		 boundaryStatus == GroundLumirrorAirReflection ||
		 boundaryStatus == GroundLumirrorGlueReflection ||
		 boundaryStatus == GroundAirReflection ||
		 boundaryStatus == GroundTeflonAirReflection ||
		 boundaryStatus == GroundTiOAirReflection ||
		 boundaryStatus == GroundTyvekAirReflection ||
		 boundaryStatus == GroundVM2000AirReflection ||
		 boundaryStatus == GroundVM2000GlueReflection ||
		 boundaryStatus == Dichroic ||
		 boundaryStatus == CoatedDielectricReflection ||
		 boundaryStatus == CoatedDielectricRefraction
		 ) {
	fEventAction->IncrementNumReflected();
      } else {
	G4cout << "[PHOTOCATHODE] Unhandled boundary status: " << boundaryStatus << G4endl;
      }
    }
  }
  */
...
void MySteppingAction::ResetStepCount()
{
  stepCount=0;
}

G4int MySteppingAction::GetStepCount() const
{
  return stepCount;

}

const std::vector<G4String>& MySteppingAction::GetVolume() const
{
  return volume;
}

const std::vector<G4String>& MySteppingAction::GetPreStepVolume() const
{
  return preStepVolume;
}

const std::vector<G4String>& MySteppingAction::GetPostStepVolume() const
{
  return postStepVolume;
}


const std::vector<G4int>& MySteppingAction::GetStepNumbers() const
{
  return stepNumbers;
}
