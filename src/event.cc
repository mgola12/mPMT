...
void MyEventAction::EndOfEventAction(const G4Event* event)
{
  ...
  */
  ...
  //  G4cout<<"TOTAL STEPS TAKEN BY PHOTON: "<<totalSteps<<G4endl;
   ...
   if(NumReflected)
     {
       ...
       for(size_t i = 0; i < volume.size(); ++i)
        {
	  eventOnTop=0;
	  //	  if(!(volume[i] == "pmtBulbLogic" || volume[i] == "pmtBaseLogic" || volume[i] == "logicLayer" || volume[i] == "absorberLogic" || volume[i] == "pmtInnerTubeLogic" || volume[i] == "pmtInnerBulbLogic"))
	  //	  if(volume[i] == "domeLogic" || volume[i] == "mPMTAirLogic")
	  if(!(volume[i] == "matrixLogic") && (preStepVolume != postStepVolume))
	     {
	       eventOnTop++;
	     }
	}
   ...
   G4cout<<"REFLECTED EVENTS: "<<  reflectedEvents<<G4endl;
   man->FillNtupleIColumn(1, 0, reflectedEvents);
   man->AddNtupleRow(1);
       /*
